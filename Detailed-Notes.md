...
- Outline - like breadcrumbs - can see the structure of the active file
- `JSDoc` 3 is an API documentation generator for JavaScript
- `CTRL` + `W` - to close an open file/tab
- `right-click` > `Find all references` but only for usable code, no comments
- `right-click` > `Find all occurences` but only for the file you are in
...
- CSS classes: Hover over the selector and you can see the specificity
- add an array to `settings.json` for things like the CDN for Tailwind or Bootstrap - search for `css style sheets` - click edit in `settings.json`
- Extension: `Import Cost` - it will calculate the size of each dependency
...
- If you hover over a etting that is a boolean or has set choices, a pencil icon appears to the left and you can change it
...
- If you want to open a folder within the folder you are in where `-r` is for reload
...
1. `CTRL` + `]` to tab entire tine
1. `CTRL` + `[` to tab entire tine backwards
...
- `Tabnine Ai Code Completion`: by tabnine.com
  - Name: Jim Kernicky, Email: gmail, PW = Luna-2023
